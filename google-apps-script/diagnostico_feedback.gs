...
function onDiagnosticoSubmit(e) {
  ...
  const ordenado = scores.slice().sort((a, b) => b.valor - a.valor);
  const fortalezas = ordenado.slice(0, 2);
  const oportunidades = ordenado.slice().reverse().slice(0, 2);
  const overallPercent = Math.round((scores.reduce((s, x) => s + x.valor, 0) / (scores.length * 4)) * 100);

  const recomendacionesTexto = oportunidades
    .map((o) => {
  ...
  const cuerpoTexto = [
    ...
    '',
    'Mapa de prioridades:',
    scores.map((s) => `${s.dimension}: ${Math.round((s.valor / 4) * 100)}%`).join('\n'),
    '',
    'Fortalezas: ' + fortalezas.map((f) => f.dimension).join(', '),
    'Oportunidades: ' + oportunidades.map((o) => o.dimension).join(', '),
    '',
  ...
  const cuerpoHtml = construirEmailHtml({ nombre, overallPercent, scores, fortalezas, oportunidades });

  Logger.log('Enviando email a %s...', email);
  MailApp.sendEmail(email, 'Tu Informe Ejecutivo Prisma®', cuerpoTexto, { htmlBody: cuerpoHtml });
  Logger.log('Email enviado correctamente.');
}

function construirEmailHtml(datos) {
  ...
  const filaBarra = (dim) => {
    ...
    return (
      ...
      '<td align="left" style="font-family:Arial,sans-serif;font-size:13px;color:' +
      TEXTO +
      ';padding-bottom:6px;">' +
      dim.dimension +
      '</td>' +
      '<td align="right" style="font-family:Arial,sans-serif;font-size:13px;color:' +
      TEXTO_SUAVE +
      ';padding-bottom:6px;">' +
      pct +
      '%</td>' +
      ...
      '%" bgcolor="' +
      AZUL_HORIZONTE +
      '" style="height:8px;line-height:8px;font-size:1px;border-radius:5px 0 0 5px;">&nbsp;</td>' +
      '<td width="' +
      (100 - pct) +
      '%" bgcolor="' +
      ARENA_CLARA +
      '" style="height:8px;line-height:8px;font-size:1px;border-radius:0 5px 5px 0;">&nbsp;</td>' +
      '</tr></table>' +
      '</td></tr>'
    );
  };

  const listaChips = (items, color) =>
    items
      .map(
        (x) =>
          ...
          ';color:' +
          AZUL_OCEANO +
          ';font-family:Arial,sans-serif;font-size:12px;font-weight:bold;padding:6px 12px;border-radius:999px;margin:0 6px 6px 0;">' +
          x.dimension +
          '</span>',
      )
      .join('');

  const recomendacionesHtml = datos.oportunidades
    .map((o) => {
      const nivel = o.valor <= 2 ? 'bajo' : o.valor === 3 ? 'medio' : 'alto';
      return (
        '<li style="margin-bottom:8px;font-family:Arial,sans-serif;font-size:14px;color:' +
        TEXTO +
        ';line-height:1.5;">' +
        RECOMENDACIONES[o.dimension][nivel] +
        '</li>'
      );
    })
    .join('');
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #E7DDC9;">
      <tr>
        <td style="background:${AZUL_OCEANO};padding:28px 32px;">
          <p style="margin:0;font-family:Arial,sans-serif;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:${ARENA};">Informe Ejecutivo Prisma&reg;</p>
          <p style="margin:10px 0 0;font-family:Arial,sans-serif;font-size:14px;color:#ffffff;opacity:0.85;">Preparado para ${datos.nombre || 'vos'}</p>
        </td>
      </tr>
      <tr>
        <td style="padding:28px 32px;">
          <p style="margin:0 0 4px;font-family:Arial,sans-serif;font-size:13px;color:${TEXTO_SUAVE};">Nivel general de claridad</p>
          <p style="margin:0 0 24px;font-family:Arial,sans-serif;font-size:36px;font-weight:bold;color:${AZUL_HORIZONTE};">${datos.overallPercent}%</p>
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            ${datos.scores.map(filaBarra).join('')}
          </table>

          <p style="margin:24px 0 8px;font-family:Arial,sans-serif;font-size:14px;font-weight:bold;color:${AZUL_OCEANO};">Fortalezas</p>
          <div>${listaChips(datos.fortalezas, ARENA)}</div>

          <p style="margin:20px 0 8px;font-family:Arial,sans-serif;font-size:14px;font-weight:bold;color:${AZUL_OCEANO};">Oportunidades</p>
          <div>${listaChips(datos.oportunidades, ARENA_CLARA)}</div>

          <p style="margin:24px 0 8px;font-family:Arial,sans-serif;font-size:14px;font-weight:bold;color:${AZUL_OCEANO};">Primeras recomendaciones</p>
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" style="margin:20px auto 4px;">
            <tr>
              <td align="center" bgcolor="${AZUL_HORIZONTE}" style="border-radius:999px;">
                <a href="${AGENDAR_URL}" style="display:inline-block;padding:14px 28px;font-family:Arial,sans-serif;font-size:14px;font-weight:bold;color:#ffffff;text-decoration:none;">Agendar mi primera conversación</a>
              </td>
            </tr>
          </table>
        </td>
